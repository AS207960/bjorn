...
The rest of Björn is appropriate for use in a production environment.
...
ACME `http-01`, `dns-01`, and `tls-alpn-01` challenges.
...
* [draft-ietf-acme-onion-03](https://datatracker.ietf.org/doc/html/draft-ietf-acme-onion-03) - Automated Certificate Management Environment (ACME) Extensions for ".onion" Special-Use Domain Names
...
Does this server only support in-band CAA for Tor Onion Services.
...
Benny should be setup behind a caching HTTP proxy that respects the `Cache-Control`, 
...
##### `tor_storage`
The directory in which Frida will store cached state for Tor Onion Services.
This is not required to be set. If not set Frida will not make connections to
the Tor network, and will only be able to validate in-band CAA for Onion Services.
...
The `onion_caa` field contains the in-band CAA records for Tor Onion Services
...
The `account_thumbprint` can be passed onto Frida to allow the validation 
to happen. Validation should happen in the background as the client will 
poll the server until the challenge succeeds or fails.
...
`invalidity_date` indicates the time at which the certificate is believed to have been compromised or otherwise invalidated (if known).
...
Frida has 4 validation methods; `ValidateHTTP01`, `ValidateDNS01`, and `ValidateTLSALPN01`, `ValidateOnionCSR01`.
They perform `http-01`, `dns-01`, `tls-alpn-01`, and `onion-csr-01` based validations respectively.
...
* `hs_private_key` - The 32-byte private key used to connect to Tor Onion Services, if applicable.
...
In the case of `dns` type identifiers the identifier value must be lowercase, non-punycode (IDNA), and without a 
...
* `hs_private_key` - The 32-byte private key used to connect to Tor Onion Services, if applicable.
* `onion_caa` - In-band CAA for Tor Onion Services, if provided by the ACME client.